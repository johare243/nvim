		...
		config = function()
			...
			local function find_apex_jar()
				local candidates = {
					...
					vim.fn.expand(
						"~/.vscode/extensions/salesforce.salesforcedx-vscode-apex-*/extension/dist/apex-jorje-lsp.jar"
					),
				}
				...
				for _, path in ipairs(candidates) do
					...
					if
						type(glob_result) == "table"
						and #glob_result > 0
						and vim.fn.filereadable(glob_result[#glob_result]) == 1
					then
			...
			if not apex_jar then
				vim.notify(
					"apex-jorje-lsp.jar not found. Install with :Mason → apex-language-server",
			...
			-- Enable LSP servers
			vim.lsp.enable({ "lua_ls", "apex_ls", "pyright" })
		end,
	},
}
